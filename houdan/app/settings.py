...
_DEFAULTS: dict[str, Any] = {
    ...
    # 顺丰丰桥开放平台凭据：配齐后，顺丰单的「待收货 → 租赁中」改由真实签收轨迹驱动，
    # 提前签收还会把归还日按真实签收重算。任一项留空 = 未对接，全量回落到
    # 「发货时间 + 物流免租期」的定时器保底（也就是对接前的老行为）。
    "sf_partner_id": "",     # 顾客编码（丰桥申请，不是月结卡号）
    "sf_check_word": "",     # 校验码，仅本地签名用，不上行
    # True = 打丰桥沙箱网关（联调用，凭据与生产不通用）；上线务必改回 False
    "sf_sandbox": False,
    # 待免押订单超时自动取消（死单清理）：
